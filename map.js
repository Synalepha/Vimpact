...
if (canvas && stage) {
  ...
  const showDetail = (entry, scroll = false) => {
    ...
    detail.innerHTML = `<p class="detail-type">${kindLabel[entry.kind]}</p><p class="detail-year">${entry.year}</p><h2>${entry.title}</h2><p class="detail-summary"><strong>Why it appears in the map:</strong> ${entry.short}</p><p>${entry.context}</p><div class="detail-pillars" aria-label="Connected pillars">${connected.map((pillar) => `<span class="pillar-${pillar.id}">${pillar.name}</span>`).join("")}</div><div class="related-nodes"><strong>Explore nearby connections</strong>${related.map((item) => `<button type="button" data-related="${item.id}">${item.year} · ${item.title}</button>`).join("")}</div><p class="detail-hint">These are thematic editorial connections, not a claim of V-Impact affiliation or endorsement.</p>`;
